...
export function DataReading({ apis, color, neon }: { apis: any, color: string, neon?:string }) {
    ...
    return (
        <article className='grid grid-cols-2 gap-4 p-8 font-tilt-neon'>
            <div className='flex flex-col gap-2 h-[50vh] overflow-auto hidden-scrollbar'>
            ...
            <div className="font-tilt-neon">
                ...
                {currentApi.value.schema.summary}
            </div>
            <div className="font-tilt-neon">
                <h1 className='text-2xl font-[100]'>Respuesta</h1>
            <div className={'p-4 border border-white rounded h-96 bg-[#0a2a3d] font-tilt-neon '}><pre>{response}</pre></div>
            </div>
